...
class BundleConfigForm implements BundleConfigFormInterface {
   ...
   *
   * @param \Drupal\localgov_finders\FinderTypeManager $finderTypeManager
  ...
  public static function submit(array $form, FormStateInterface $form_state): void {
    ...
    if (!empty($form_state->getValue(['localgov_finders', 'finder_type'])) && !empty($form_state->getValue(['localgov_finders', 'finder_role']))) {
      $bundle_type->setThirdPartySetting(
        'localgov_finders',
        'finder_type',
        $form_state->getValue(['localgov_finders', 'finder_type'])
      );
      $bundle_type->setThirdPartySetting(
        'localgov_finders',
        'finder_role',
        $form_state->getValue(['localgov_finders', 'finder_role'])
      );
      $bundle_type->save();
    }
  }
